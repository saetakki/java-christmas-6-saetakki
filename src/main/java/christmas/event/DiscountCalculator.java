...
import static christmas.event.VisitDayDiscountUntilChristmas.applyAdditionalDiscount;
...
public class DiscountCalculator {
    ...
    private static int calculateTotalDiscountedPrice(Map<String, Integer> cart, Integer date) {
        int basicDiscountAppliedPrice = cart.entrySet().stream().map(calculatePriceWithDiscountStrategy(date)).reduce(0, (a, b) -> a + b);
        Function<Integer, Integer> AdditionalDiscountApplier = applyAdditionalDiscount(date);
    ...
    private static Function<Map.Entry<String, Integer>, Integer> calculateDiscountedPricePerItem =
            entry -> (getPriceOfMenu(entry.getKey())-DISCOUNT_AMOUNT) * entry.getValue();
